...
public class BorrowBook extends javax.swing.JFrame {
    ...
    public BorrowBook(BookStorage storage, UserStorage userstorage) {
        initComponents();
    ...
    private void initComponents() {
        ...
        jButton8.setBackground(new java.awt.Color(183, 223, 243));
    ...
    private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed
        ...
        try {
            ...
            if (pages <= 0) {
                JOptionPane.showMessageDialog(this, "Pages must be more than 0.");
                return;
            }
    ...
    // If text field is blank, show the popup list
    if (query.isEmpty()) {
        java.util.ArrayList<String> availableTitles = new java.util.ArrayList<>();
        for (int i = 0; i < bookStorage.getCount(); i++) {
            Book b = bookStorage.getAllBooks()[i];
            if (!b.isBorrowed()) {
                availableTitles.add(b.getTitle() + " - " + b.getAuthor());
            }
        }
        if (availableTitles.isEmpty()) {
            JOptionPane.showMessageDialog(this, "No books available to borrow.");
            return;
        }
        String[] options = availableTitles.toArray(new String[0]);
    ...
    // Borrow the book by title (typed or selected)
    boolean found = false;
    for (int i = 0; i < bookStorage.getCount(); i++) {
        Book b = bookStorage.getAllBooks()[i];
        if (b.getTitle().equalsIgnoreCase(query)) {
            if (b.isBorrowed()) {
                ...
                b.setBorrowed(true);
                JOptionPane.showMessageDialog(this, "You have successfully borrowed:\n" + b.getTitle() + " by " + b.getAuthor());
            }
            found = true;
            break;
        }
    ...
    if (!found) {
        JOptionPane.showMessageDialog(this, "Book not found in library.");
    }
